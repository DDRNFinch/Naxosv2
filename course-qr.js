...
(function(){
  ...
  function renderQr(course){
    ...
    if(!header || header.querySelector('.naxos-qr')) return;
    ...
    new QRCode(wrap.querySelector('.naxos-qr-code'), {
      text: coursePayload(course),
      width: 132,
      height: 132,
      colorDark: '#000000',
      colorLight: '#ffffff',
  ...
  function patchOpenCourse(){
    if(typeof window.openCourse !== 'function' || window.openCourse.__naxosQrPatched) return false;
    var original=window.openCourse;
    function patched(id){
      original(id);
      var course=window.metadata && window.metadata.courses ? window.metadata.courses[id] : null;
      renderQr(course);
    }
    patched.__naxosQrPatched=true;
    window.openCourse=patched;
    return true;
  }
  ...
  var timer=setInterval(function(){
    attempts++;
    if(patchOpenCourse() || attempts>100) clearInterval(timer);
  },50);
})();
